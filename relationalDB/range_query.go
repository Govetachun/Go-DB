...
func Seek(tree *btree.BTree, key []byte, cmp int) *btree.BIter {
...
func (sc *Scanner) Deref(rec *Record) {
	utils.Assert(sc.Valid(), "sc.Valid()")
	tdef := sc.tdef
	rec.Cols = tdef.Cols
	rec.Vals = rec.Vals[:0]
	...
	if sc.indexNo < 0 {
		...
		utils.Assert(len(val) == 0, "len(val) == 0")
		...
		utils.Assert(ok && err == nil, "ok && err == nil")
	}
}
...
func dbScan(db *DB, tdef *TableDef, req *Scanner) error {
	...
	req.iter = Seek(db.kv, keyStart, req.Cmp1)
	return nil
}
...
func dbGetRange(db *DB, tdef *TableDef, rec *Record) (bool, error) {
